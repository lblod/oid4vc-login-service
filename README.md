...
- **VERIFIER_UNSIGNED**: whether to use unsigned authorization responses in the verifier, defaults to false. This sets the response mode used in the verifier (see OID4VP protocol), to direct_post and the client_id prefix to redirect_uri. This means the verification request cannot be signed as there is no way for the wallet to obtain a trusted key for verification. Normally the response mode is set to direct_post.jwt and the client_id prefix to decentralized_identifier, as is recommended for extra security, but this is unsupported by the EUDI wallet.
