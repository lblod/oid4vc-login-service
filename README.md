...
- **VERIFIER_RESPONSE_MODE**: response mode used in the verifier (see OID4VP protocol), defaults to direct_post.jwt. Can be set to direct_post instead, in which case the client_id prefix will be set to redirect_uri and the service's response_uri is used as client id. This means the verification request cannot be signed as there is no way for the wallet to obtain a trusted key for verification. direct_post.jwt is recommended for extra security, but unsupported by the EUDI wallet.
